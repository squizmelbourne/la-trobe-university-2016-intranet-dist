 ...
 * change log:
 *     Fri Mar 17 2017 16:30:34 GMT+1100 (AEDT) - First revision
 */
        ...
        return new jQuery.Deferred(function (promise) {
            jsAPI.getMetadata({
                ...
                'dataCallback': function dataCallback(obj) {
                    if (obj.errorCode && obj.errorCode === 'permissionError') {
                        // Fallback for LDAP users which are not allowed to get their own metadata through the API
                        //promise.resolve(
                        // $('#' + metadataName.replace(/\./g,'-')).val().replace(/[\[\]']+/g, '').split(',')
                        //);
                        console.log('fires');
                        makeAJAXCall('GET', recacheUrl, renderMetadata());
                    } else {
    ...
    */
    ...
    function setMetadataNoApi(userId, currentFieldID, userMeta, targetDOMel, loadingFlag) {
        ...
        makeAJAXCall('GET', ajaxProxyUrl + '?uid=' + userId + '&fid=' + currentFieldID + '&fval=' + userMeta, setTimeout(function () {
            makeAJAXCall('GET', recacheUrl + '/_recache', setLoadingState(targetDOMel, loadingClass, loadingFlag));
        ...
        console.log('fetch : fetchMetadata');
        fetchMetadata(userId, metaName);
    }
