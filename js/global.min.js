 ...
 * change log:
 *     Thu Mar 02 2017 12:04:07 GMT+1100 (AEDT) - First revision
 */
    ...
    init: function init() {
        if (feedback.config.feedbackForm) {
            ...
            document.querySelector('body').addEventListener('click', function (ev) {
                ...
                var clickedElClass = ev.target.getAttribute('class').split(' ')[0];
    ...
    if (searchForm) {
        ...
        if (currentQs.length) {
            ...
            var mappedHiddenFields = currentQs.split('&').reduce(function (acc, el) {
                var key = el.split('=')[0];
                var val = el.split('=')[1];
                return key.indexOf('query') === -1 ? acc + '<input type="hidden" name="' + key + '" value="' + val + '" />' : acc;
            }, '');
