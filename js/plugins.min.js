 ...
 * modified: Mon Mar 27 2017 09:56:38 GMT+0200 (CEST)
 * @preserve
 */
    ...
    NavBar.prototype = {
        ...
        init: function init() {
            ...
            this.$items.each(function (i, item) {
                ...
                $item.on('click.' + pluginName, function (event) {
                    //Toggle state if the clicked element is the top level
                    // if (event.target.closest('.sub-mega-nav') === null) {} // IE don't support closset
                    if (event.target.parentNode.querySelector('.sub-mega-nav')) {
                        event.preventDefault();
                        self.toggleState($item);
                    }
                });
            });
            ...
            $('body').on("click." + pluginName, function (event) {
                if (event.target.parentNode.parentNode.querySelector('.navbar__list')) {
                    self.clearState();
                }
            });
        },
        ...
        setState: function setState($item, active) {
            ...
            if (active) {
                ...
                $('body').on('click.megamenu', function (event) {
                    function clickOutside(targetClassList) {
                        if (document.querySelector('.nav-main--desktop').querySelector(targetClassList)) {
                            // Check if the clicked element is the nav user condensed. If so, despite of being part of
                            // the .nav-main--desktop, we should close it anyway.
                            return $(targetClassList).closest('.nav-user').length ? true : false;
                        }
                        // If it reaches this point is because the clicked element is outside of the nav main.
                        // Returning true to close it.
                        return true;
                    }

                    var targetClassList = Array.from(event.target.classList).reduce(function (acc, el) {
                        return acc + '.' + el;
                    }, '');
                    var clickedOutisde = clickOutside(targetClassList);

                    if (clickedOutisde) {
                        _this.closeMegaMenu($item, active, 'click.megamenu');
                    }
                });
            } else {
                _this.closeMegaMenu($item, active, 'click.megamenu');
        ...
        closeMegaMenu: function closeMegaMenu(item, active, custEventName) {
            item.removeClass(this.settings.classes.itemActive);
            $('.' + this.settings.classes.sub, item).attr({
                'aria-expanded': 'false',
                'aria-hidden': 'true'
            });
            $('body').off(custEventName);
        },
        isActive: function isActive($item) {
            return $item.hasClass(this.settings.classes.itemActive);
        },
...
PageOverlay = {
    ...
    defaults: {
        overlayID: 'page-overlay',
        defaultClass: 'page-overlay'
    },
    /*
     * Hide page overlay.
     */
    hide: function hide() {
        var overlay = document.getElementById(PageOverlay.defaults.overlayID);
        if (overlay) {
    ...
    init: function init() {
        ...
        if (!overlay) {
            ...
            overlay.setAttribute('id', PageOverlay.defaults.overlayID);
            overlay.classList.add(PageOverlay.defaults.defaultClass);
...
(function (factory) {
    ...
    Selectric.prototype = {
        ...
        getItemMarkup: function getItemMarkup(i, elm) {
            ...
            return _this.utils.format('<li data-index="{1}" class="{2}">{3}</li>', i, _this.utils.arrayToClassname([elm.className, i === _this.items.length - 1 ? 'last' : '', elm.disabled ? 'disabled' : '', elm.selected ? 'selected' : '']), $.isFunction(itemBuilder) ? itemBuilder(elm, elm.element, i) : _this.utils.format(itemBuilder, elm));
        },
        ...
        select: function select(index) {
            ...
            if (_this.state.multiple) {
                ...
                $filteredLi.filter(function (index) {
                    return $.inArray(index, _this.state.selectedIdx) !== -1;
                }).addClass('selected');
            } else {
                $filteredLi.eq(_this.state.selectedIdx = index).addClass('selected');
            }
    ...
    $.extend($.validator, {
        ...
        prototype: {
            ...
            element: function element(_element2) {
                var cleanElement = this.clean(_element2),
                    checkElement = this.validationTargetFor(cleanElement),
                    v = this,
                    result = true,
                    rs,
                ...
                if (checkElement === undefined) {
                    ...
                    // Add aria-invalid status for screen readers
                    $(_element2).attr("aria-invalid", !rs);
                }
